...
const url = '/api/skbp1/getvolume?prodi=informatika&type=Jurnal';
...
getVolume()
        ...
        function getListFromVolume(volume,paginate){
          ...
          let url;
          if(volume){
            url ='/api/skbp1/getList?vol='+volume+'&prodi=informatika&type=jurnal';
          }else if(paginate){
            url =paginate;
          }else{
            url ='/api/skbp1/getList?prodi=informatika&type=jurnal';
          }
...
function searchJurnal(search){
  fetch(`/api/skbp1/search?prodi=informatika&type=Jurnal&search=${search}`)
