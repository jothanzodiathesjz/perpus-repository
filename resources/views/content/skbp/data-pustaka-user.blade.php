...
<h4 class="fw-bold py-3 mb-4">
  <span class="text-muted fw-light">Dashboard/</span> Data Pustaka
</h4>
  ...
  <div class="row">
      ...
      <div class="col-md col-lg-12">
        <div class="card invoice-preview-card">
          <div class="card-body">
            <h3 id="buletin-title">My Data Pustaka</h3>
...
const url = '/api/skbp1/getvolume?prodi=Teknik Elektro&type=Skripsi';
...
// getVolume()
        ...
        function getListFromVolume(volume,paginate){
          ...
          let url;
          if(volume){
            url ='/api/skbp1/getList?vol='+volume+'&prodi=Teknik Elektro&type=Skripsi';
          }else if(paginate){
            url =paginate;
          }else{
            url ='/api/pustaka/{{auth()->user()->id}}';
          }
        ...
        fetch(url)
          ...
          .then(data => {
            ...
            <div class="">
              <h5 class="mb-0">
              <a href="/admin/skbp1/${element.id}?no=${index + 1}">${element.judul}</a>
              </h5>
              <div class="artifact-info">
              ...
              <div class="artifact-info">
              <span class="author">Type : <small>${element.type}</small> <small></small></span>
              </div>
          </div>
              `)
            })
...
function searchJurnal(search){
  fetch(`/api/pustaka/{{auth()->user()->id}}?search=${search}`)
